...
void PowerPlant::SetPlantBudget(){
    ...
    while (Flag == false) {
        ...
        std::cout<<"Set Matinance Budget: ";
        std::cin >> MatinanceBudget;
        std::cin.clear();
        ...
        if (atoi(InvestBudget.c_str())+atoi(MatinanceBudget.c_str())+atoi(PRBudget.c_str())+atoi(StaffBudget.c_str()) > Budget) {
        ...
        if (atoi(InvestBudget.c_str())+atoi(MatinanceBudget.c_str())+atoi(PRBudget.c_str())+atoi(StaffBudget.c_str()) <= Budget) {
            while (Continue != false) {
                ...
                if (Confirm== "yes" || Confirm== "Yes" || Confirm== "y"||Confirm== "Y") {
                    ...
                    MatinanceBudget = atoi(MatinanceBudget.c_str());
...
void PowerPlant::getAssistantManager(){
    ...
    MITCollegeGrad.MatinanceAbility = MITCollegeGrad.MatinanceAbility+7;
    ...
    for (int i=0; i<5; i++) {
        ...
        std::cout<<"Plant Matinance Ability: "<<Applications[i].MatinanceAbility<<std::endl;
