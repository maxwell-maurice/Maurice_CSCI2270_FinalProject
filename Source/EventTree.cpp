...
void EventTree::RunEvent(int ID){
    EventNode* Event = searchEventTree(root,ID);
    if (Event != NULL){
        std::cout << "Movie Info:" << std::endl;
        std::cout << "===========" << std::endl;
        std::cout << "Ranking:" << Event->InvestmentChange << std::endl;
        std::cout << "Title:" << Event->PublicRelationsChange << std::endl;
        std::cout << "Year:" << Event->PowerProductionChange << std::endl;
        std::cout << "Quantity:" << Event->BoardChange << std::endl;
    }
    else
...
void EventTree::EventHandler(PowerPlant UserPlant, EventNode* CurrentEvent, AssistantManager AM){
    ...
    if (CurrentEvent->PowerProductionChange != 0) {
        CurrentEvent->Marked=true;
        std::cout<<CurrentEvent->Text<<"\n"<<std::endl;
        if ((CurrentEvent->PowerProductionChange)/10 >= (UserPlant.MatinanceBudget)/(10-AM.MatinanceAbility)) {
            std::cout<<"Unfortuantly Your budget for Investment was not high enough"<<std::endl;
            std::cout<<"and your Assistaint manager could not resolved the situation"<<std::endl;
